...
describe("Get Balance Controller", () => {
  ...
  it("Should /GET /api/v1/statements/balance to recover user balance", async () => {
    const mocked_user = {
      name: "teste",
  ...
  it("Should /GET /api/v1/statements/balance and dont recover balance of non authenticated user", async () => {
    const balanceResponse = await request(app)
      .get("/api/v1/statements/balance")
      .send();

    expect(balanceResponse.status).toBe(401);
    expect(balanceResponse.body).toStrictEqual({
