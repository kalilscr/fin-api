...
describe("Get Statement Operation Controller", () => {
  ...
  it("Should /GET /api/v1/statements/:statement_id to recover statement operation", async () => {
    ...
    const statementOperationResponse = await request(app)
  ...
  it("Should /GET /api/v1/statements/:statement_id and dont recover statement operation from non authenticated user", async () => {
    ...
    expect(statementOperationResponse.body).toStrictEqual({
      message: "JWT token is missing!",
    });
  });

  it("Should /GET /api/v1/statements/:statement_id and dont recover non existent statement operation", async () => {
    const authenticationResponse = await request(app)
      .post("/api/v1/sessions")
      .send({ email: mocked_user.email, password: mocked_user.password });
    const { token } = authenticationResponse.body;
